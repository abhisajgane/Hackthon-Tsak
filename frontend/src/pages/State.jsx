...
const State = () => {
  ...
  const getState = async () => {
    try {
      const response = await axios.get(`${Base_Url}/api/get/state`, {
        headers: {
  ...
  const handleAddSave = async () => {
    try {
      await axios.post(`${Base_Url}/api/add/state`, newState, {
        headers: {
          "Content-Type": "application/json",
          Authorization: accessToken,
        },
  ...
  const handleEditSave = async () => {
    try {
      const response = await axios.patch(
        `${Base_Url}/api/update/state/${selectedState._id}`,
        {
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`${Base_Url}/api/delete/${selectedState._id}`, {
        headers: {
          Authorization: accessToken,
        },
      });
    ...
    <div className="container">
      <div className="flex justify-between">
        <div className="flex gap-6 justify-center align-center mb-4">
          <img src={state} alt="state" width={"60px"} height={"30px"}/>
    ...
    {filteredData.map((state, index) => (
      <tr key={state._id}>
        ...
        <td className="flex gap-4">
          ...
          <button onClick={() => handleDelete(state)}>
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
